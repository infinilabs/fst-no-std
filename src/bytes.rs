...
/// Write a u32 in little endian format to an `FstWrite` implementor.
#[inline]
#[cfg(feature = "alloc")]
pub fn io_write_u32_le<W: FstWrite + ?Sized>(n: u32, wtr: &mut W) -> Result<()> {
    let mut buf = [0; 4];
    write_u32_le(n, &mut buf);
    wtr.fst_write_all(&buf)
}
...
/// Write a u64 in little endian format to an `FstWrite` implementor.
#[inline]
#[cfg(feature = "alloc")]
pub fn io_write_u64_le<W: FstWrite + ?Sized>(n: u64, wtr: &mut W) -> Result<()> {
    let mut buf = [0; 8];
...
#[inline]
#[cfg(feature = "alloc")]
pub fn pack_uint<W: FstWrite + ?Sized>(wtr: &mut W, n: u64) -> Result<u8> {
...
#[inline]
#[cfg(feature = "alloc")]
pub fn pack_uint_in<W: FstWrite + ?Sized>(
    wtr: &mut W,
    mut n: u64,
    nbytes: u8,
) -> Result<()> {
    ...
    wtr.fst_write_all(&buf[..nbytes as usize])?;
    Ok(())
}
